...
import { getAllPropertiesAdmin, approveProperty, deleteProperty, createProperty } from '../../services/api';
    ...
    <div style={{ padding: '32px' }}>
      ...
      {showForm && (
        <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '24px', border: '1px solid #E8D5B7' }}>
          <h3 style={{ marginBottom: '16px', color: '#1A1612' }}>➕ Create New Property</h3>
          <form onSubmit={createPropertyHandler} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        ...
        {filteredProperties.map(property => (
          <div key={property.id} style={{ background: 'white', borderRadius: '16px', border: '1px solid #E8D5B7', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => deletePropertyHandler(property.id)} style={{ padding: '8px 16px', background: '#C62828', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>🗑️ Delete</button>
            </div>
          </div>
        ))}
      </div>
